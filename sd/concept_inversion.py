...
def random_crop_square(image_path, d):
    """
    Randomly crops a square of dimensions d x d from an image with dimensions m x n (d < m and d < n).
    
    Parameters:
        image_path (str): Path to the image file.
        d (int): Dimension of the square to crop (d x d).
        
    Returns:
        cropped_image (PIL.Image.Image): Cropped image of dimensions d x d.
    """
    ...
    # Check that d < m and d < n
    if not (d < m and d < n):
        raise ValueError("Invalid dimensions. Make sure that d < m and d < n.")
    
    # Calculate random position for cropping
    left = random.randint(0, m - d)
    upper = random.randint(0, n - d)
...
class TextualInversionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # if(n > 512 and m > 512):
        #     image = random_crop_square(self.image_paths[i % self.num_images], 512)
